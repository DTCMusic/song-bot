...
async def send_msg(user_id, message):
    try:
        ...
        return 400, f"{user_id} : botu blokladı\n"
...
async def broadcast(m, db):
    ...
    if failed == 0:
        await m.reply_text(
            text=f"Yayımı tamamlandı `{completed_in}`\n\nToplam istifadəçi {total_users}.\nGöndərildi {done}, {success} uğur və uğursuz {failed}",
            quote=True,
        )
    else:
        await m.reply_document(
            document="broadcast.txt",
            caption=f"Yayımı tamamlandı `{completed_in}`\n\nToplam istifadəçi {total_users}.\nGöndərildi {done}, {success} uğur və uğursuz {failed}",
            quote=True,
        )
