...
@app.message_handler(commands = ['song'])
...
@dp.message_handler(
    lambda message: message.text not in config.commands and not message.text.startswith("/newpost"))
...
@app.callback_query_handler(lambda call: call.data in ("to_left", "close", "to_right"))
...
@app.callback_query_handler(lambda call: call.data in ["like", "unlike"])
...
@app.callback_query_handler(lambda call: call.data in ["select_ru", "select_en", "select_es"])
...
@app.message_handler(commands = ['artist'])
...
@app.message_handler(commands = ['setlang'])
...
@app.message_handler(commands = ['settings'])
...
@app.message_handler(commands = ['my'])
...
@app.callback_query_handler(lambda call: call.data == "to_right_playlist")
...
@app.callback_query_handler(lambda call: call.data in ["change_lang", "count_result", "heart_buttons"])
...
@app.message_handler(commands = ['newpost'])
...
@app.message_handler(commands = ['users'])
...
@app.callback_query_handler(lambda call: call.data.startswith("playlist"))
async def select_sound(call: aiogram.types.CallbackQuery):
    get_song_num = call.data.split('_')
    playlist = users[call.from_user.id]['favourites_list']
    song_num = int(get_song_num[1]) - 1
